...
mod types;
pub use types::{SessionInfo, SessionOption, TaskInfo, WorkspaceOption, WorkspaceRequest, SessionRequest, RenameSessionRequest, SearchSessionsRequest};
use types::{AppConfig, RunningTask, RuntimeState, SessionFile, SessionIndex, SessionSummary, StreamEvent};
...
fn emit_stream_error(app: &AppHandle, session_id: &str, content: &str) {
    let debug = format_stream_error_debug(content);
    emit_stream(
        app,
        StreamEvent {
...
fn format_stream_error_debug(content: &str) -> String {
    ai::stream_error_debug(content)
}
